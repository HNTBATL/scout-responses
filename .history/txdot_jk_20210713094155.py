...
trip_csv = []
trip_csv.append(['first_name', 'last_name', 'zip', 'going_to_x', 'going_to_y', 'starting_from_x', 'starting_from_y'])
...
for response in survey_responses:
    ...
    print('COMMENT LOCATION______________________GOING TO:')
    ...
    stakeholder_last_name = staimport csv
import json
import requests
...
#     sce_type_writer.writerows(type_csv)keholder['last_name'].split('-')[0]
    ...
    response_list[headers.index('last_name')] = stakeholder_last_name
    response_list[headers.index('zip_code')] = stakeholder_zip_code
    trip_csv.append((stakeholder_first_name, stakeholder_last_name, stakeholder_zip_code, x, y))
    ...
    trip_csv.append((stakeholder_first_name, stakeholder_last_name, stakeholder_zip_code, going_to_x, going_to_y, starting_from_x, starting_from_y))
    for question, answer in response['response_json'].items():
        response_list[headers.index(question)] = answer
    to_csv.append(response_list)
