...
def pull_density_points(pnt_list):
    ...
    for pnt in pnt_list:
        ...
        # else:
        #     pass
        print('65: ', pnt, pnt_list[pnt])
        if pnt == 'starting_from':
            from_coords.append([pnt_list[pnt]['x'], pnt_list[pnt]['y']])
        if pnt == 'going_to':
            to_coords.append([pnt_list[pnt]['x'], pnt_list[pnt]['y']])
    xy_coords['starting_from'] = from_coords
    xy_coords['going_to'] = to_coords
    return xy_coords

# print('LAST SURVEY RESPONSE:', survey_responses[survey_count-1])

for response in survey_responses:
    comment_location = response['comment_location']
    xy_coords = pull_density_points(comment_location)
    print('80: ', xy_coords)
