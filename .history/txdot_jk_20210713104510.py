...
def pull_type_points(pnt_list, direction):
    coords_list = []
    xy_coords = {}
    for pnt in pnt_list:
        xy_coords[direction] = coords_list.append(pnt['x'], pnt['y'])
    return xy_coords
...
for response in survey_responses:
    ...
    from_coords = pull_type_points(comment_location_from, "starting_from")
    print(from_coords)
    comment_location_to = response['comment_location']['going_to'] # yields array
    to_coords = pull_type_points(comment_location_to, "going_to")
    # print(type_coords)
