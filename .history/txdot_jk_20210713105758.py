...
# type_csv.append(['first_name', 'last_name', 'zip', 'from_coords', 'to_coords'])

# def pull_type_points(pnt_list):
#     coords_list = []
#     # xy_coords = {}
#     for pnt in pnt_list:
#         coords_list.append([pnt['x'], pnt['y']])
#     return coords_list

# for response in survey_responses:
#     comment_location = response['comment_location']
#     comment_location_from = response['comment_location'][0] # yields array
#     print(comment_location_from)
#     from_coords = pull_type_points(comment_location_from)
#     # print(from_coords)
#     comment_location_to = response['comment_location'][1] # yields array
#     to_coords = pull_type_points(comment_location_to)
#     # print(type_coords)
...
#     type_csv.append((stakeholder_first_name, stakeholder_last_name, stakeholder_zip_code, from_coords, to_coords))
...
def pull_type_points(pnt_list):
    print('LINE 147______________________________________________________')
    print(pnt_list['going_to'])
    coords_list = []
    # xy_coords = {}
    for pnt in pnt_list:
        coords_list.append([pnt['x'], pnt['y']])
...
for response in survey_responses:
    comment_location = response['comment_location']
    coords = pull_type_points(comment_location)
    # print(type_coords)
    survey_taker_id = response['survey_taker_id']
    stakeholder = [sh for sh in stakeholders if sh['pk'] == survey_taker_id][0]
    stakeholder_first_name = stakeholder['first_name'].split('-')[0]
