...
trip_csv = []
trip_csv.append(['first_name', 'last_name', 'zip', 'going_to', 'starting_from'])
...
for response in survey_responses:
    comment_location_to = response['comment_location']['going_to'] # yields array
    comment_location_from = response['comment_location']['starting_from'] # yields array
    to_coords = pull_points(comment_location_to)
    from_coords = pull_points(comment_location_from)
    ...
    response_list = [""] * len(headers) # create placeholders
    response_list[headers.index('from_coords')] = from_coords
    response_list[headers.index('to_coords')] = to_coords
    ...
    trip_csv.append((stakeholder_first_name, stakeholder_last_name, stakeholder_zip_code, to_coords, from_coords))
    # going_to = comment_location.get('going_to', (0, 0))
    # starting_from = comment_location.get('starting_from', (0, 0))
    # trip_csv.append((stakeholder_first_name, stakeholder_last_name, stakeholder_zip_code, going_to, starting_from))
